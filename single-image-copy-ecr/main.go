...
var env = struct {
	...
	SourceImage     string `envconfig:"SOURCE_IMAGE" required:"true"`
}{}
...
func handler(ctx context.Context, levent events.LambdaFunctionURLRequest) (resp string, err error) {
	...
	if !matchesSelectedImage(repoName) {
		log.Printf("repo %q does not match SOURCE_IMAGE %q; skipping", repoName, env.SourceImage)
		return "", nil
	}
...
func matchesSelectedImage(repoName string) bool {
	selected := strings.TrimSpace(env.SourceImage)
	selected = strings.TrimPrefix(selected, "cgr.dev/")
	selected = strings.TrimPrefix(selected, "https://cgr.dev/")
	if selected == "" {
		return false
	}
	fullRepo := env.GroupName + "/" + repoName
	return selected == repoName || selected == fullRepo
}
